...
## 11. Feature Graphic — DONE (June 20, 2026)

✅ Generated: `frontend/assets/images/allsale-feature-graphic.png`
- **Size:** 1024×500 PNG (Play Store requirement met)
- **Mode:** opaque RGB, ~78 KB
- **Design:** brand-orange diagonal gradient (peach `#FFC38C` → deep orange `#F37021`), white rounded card on left containing the wordmark, bold 58pt headline "Shop India. Delivered home." on right, 22pt supporting line listing destinations.

Upload directly to Play Console → Store Presence → Main Store Listing → Graphics → "Feature graphic" slot.

If you need a v2 you can re-run the generator script (lives inline in the chat history) — easy to swap copy, gradient colours, or destination list.


---

## 12. Review Account — DONE (June 20, 2026)

✅ Reviewer account live and exercise-ready.

**Credentials** (paste into App Store Connect / Play Console "Sign-in info"):
- Email: `apple-review@allsale.co.nz`
- Password: `AppleReview2026!`

**Seeded data the reviewer will see immediately:**
- 1 paid + shipped order (`ord_…`) — 2 items, NZD $105.02, Shiprocket X tracking `SRX-DEMO-001` with 4 transit events.
- Cupertino shipping address (1 Infinite Loop) — instantly recognisable to Apple reviewers.
- 1 wishlist item so the Wishlist tab isn't empty.

**Re-seed any time:**
```bash
cd /app/backend && python -m scripts.seed_review_account
```
The script is idempotent: it re-creates the account if missing, replaces seeded rows on every run (matched by `_seed: "review_account"`).

**Suggested App Review Notes (paste into App Store Connect):**
> Allsale ships physical goods from India to NZ/AU/US/UK/CA. No in-app purchases — payments handled by Stripe (use test card 4242 4242 4242 4242 if you wish to complete a purchase). The reviewer account is pre-seeded with a shipped order so you can exercise the order-tracking flow without making a purchase. Account deletion is available at Account → Privacy → Delete Account.
